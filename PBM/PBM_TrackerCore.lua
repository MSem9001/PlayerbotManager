...
local function OnFirstShow()
    ...
    orphanedBotsBtn:SetScript("OnEnter", function()
        ...
        GameTooltip:AddLine("group or raid, and removes them", 0.8, 0.8, 0.8)
        GameTooltip:AddLine("from the tracker as well.", 0.8, 0.8, 0.8)
        GameTooltip:Show()
    end)
    ...
    PBM.RemoveOrphanedBots = function(onComplete)
        ...
        orphanFrame:SetScript("OnUpdate", function(_, elapsed)
            ...
            -- Orphaned bots are, by definition, not in your group/raid, so they
            -- can be anywhere in the world and out of SAY range. Whisper the
            -- command straight to the bot by name (same pattern PBM.SendToBot
            -- uses for other per-bot commands) so it reaches them regardless
            -- of distance, instead of a SAY broadcast that only nearby bots hear.
            SendChatMessage(".playerbots bot remove "..bname, "WHISPER", nil, bname)
